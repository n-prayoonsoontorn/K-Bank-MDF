...
/*---------------------------------------------------------------------------------------------------
# Name: tbl_init_raw_rmst_customer.sql
# Area: rmst
#----------------------------------------------------------------------------------------------------
#
# Change Revision
...
#----------------------------------------------------------------------------------------------------
# 2024-03-11    Tanatchporn S.         Initial Version
#
# Target table(s)/view(s): ${catalog}.RAW_RMST.RMST_CUSTOMER
#--------------------------------------------------------------------------------------------------*/

-- COMMAND --

create table if not exists ${catalog}.raw_rmst.rmst_customer (
...
Def(Th): ชื่อลูกค้า",
	createddate                                               	string        comment "Def(En): Record's create date
Def(Th): วันที่สร้าง Record",
	lastmodifieddate                                          	string        comment "Def(En): Record's last modified date
...
Def(Th): ละติจูดและลองติจูดของที่อยู่ลำดับที่ 1",
	af_last_callreprt_opty_updated_date__c                    	string        comment "Def(En): AF Latest Call Report Date
...
Def(Th): ชื่อเขตของทีม SME",
	assetcurrentasset__c                                      	string        comment "Def(En): Current Asset
...
Def(Th): มูลค่าทรัพย์สินสุทธิของกองทุน 6 เดือนของ Private Banking",
	aum__c                                                    	string        comment "Def(En): Asset under Management
...
Def(Th): ข้อมูลลูกค้า Bad เพิ่มเติม",
	benefit_spent__c                                          	string        comment "Def(En): Customer's Benefit Spent
...
Def (Th): ข้อมูลของ Call Report ใบล่าสุด",
	capacity_max_reward_amount__c                             	string        comment "Def (En): Number of max rewards amount
...
Def(Th): ข้อมูลลีดที่เกี่ยวข้องกับลูกค้า",
	changed_owner_channel__c                                  	string        comment "Def(En): A field used to verify if the change owner command came from the following legit channels:
1 : HRIS
2 : Bulk Assignment
3 : Request/Release
4 : Manual Upload
...
Def(Th): ช่องทางการติดต่อลูกค้า",
	cheque_solution_lastestdate__c                            	string        comment "Def(En): Latest Order Cheque Solution Date
Def(Th): วันที่สั่งเช็ค Solution ล่าสุด",
	cheque_solution_number__c                                 	string        comment "Def(En): Number of Cheque Solution
...
Def(Th): กลุ่มสีของเจ้าของลูกค้า",
	commercial_exposure__c                                    	string        comment "Def(En): Sum of  Loan out+Limit (OD,PN,TF,LG,Aval,Acceptance )
Def(Th): ผลรวมของ Loan out+Limit (OD,PN,TF,LG,Aval,Acceptance )",
	commercial_limit__c                                       	string        comment "Def(En): Approve Amount of LN,OD,PN,TF,LG,Aval,Acceptance
Def(Th): วงเงินอนุมัติของ LN,OD,PN,TF,LG,Aval,Acceptance",
	commercial_out__c                                         	string        comment "Def(En): Outstanding Balance of LN,OD,PN,TF,LG,Aval,Acceptance
Def(Th): ยอดคงค้างของ LN,OD,PN,TF,LG,Aval,Acceptance",
	consumer_exposure__c                                      	string        comment "Def(En): Sum of Home Loan out and Limit  PL,CC,KEC
Def(Th): ยอดรวม Home Loan out+Limit ของ PL,CC,KEC",
	consumer_limit__c                                         	string        comment "Def(En): Approve Amount of Home LN,PL,CC,KEC 
Def(Th): วงเงินอนุมัติของ Home LN,PL,CC,KEC",
	consumer_out__c                                           	string        comment "Def(En): Outstanding Balance of Home LN,PL,CC,KEC
...
Def(Th): คนที่ทำการแก้ไข Contact Preference ล่าสุด",
	contact_preference_last_modified_date__pc                 	string        comment "Def(En): Contact Preference Last Modified Date
...
Def(Th): ประเภทตัวแทนของผู้ติดต่อ",
	convert_prospect_date__c                                  	string        comment "Def(En): Convert Prospect Date
Def(Th): วันที่มีการ Convert Prospect ไปเป็นลูกค้า",
	count_call_report_rm__c                                   	string        comment "Def(En): Number of Relationship Mangert Call Report
Def(Th): จำนวน Call Report ของ Relationship Manager",
	count_contacted_call_report__c                            	string        comment "Def(En): Number of Contacted Call Report
Def(Th): จำนวน Call Report ที่ติดต่อลูกค้าไปแล้ว",
	count_contacted_call_report_f2f__c                        	string        comment "Def(En): Number of Face to Face Call Report
Def(Th): จำนวน Call Report แบบ Face to Face",
	count_contacted_call_report_phone_call__c                 	string        comment "Def(En): Number of Phone Call Call Report
...
Def(Th): รหัสพนักงานของเจ้าของลูกค้า",
	current_ratio__c                                          	string        comment "Def(En): Current Ratio
...
Def(Th): ชื่อเจ้าของลูกค้า",
	customer_owner_changed_date__c                            	string        comment "Def(En): Customer Owner Changed Date
...
Def(Th): ใช้อธิบายประเภทของลูกค้านิติบุคคล 01  บริษัทจำกัด, 02  ห้างหุ้นส่วนจำกัด and etc",
	de_ratio__c                                               	string        comment "Def(En): Debt to Equity Ratio 
...
Def(Th): Sub Segment Code ของ Dual",
	ebanlastmodifieddate__c                                   	string        comment "Def(En): The date that identify the data being updated from Sales Smart into DIH
...
Def(Th): ชื่อ Email ของลูกค้า",
	estimate_sale__c                                          	string        comment "Def(En): Estimate Sale Per Year
...
Def(Th): กีฬาที่ลูกค้าชอบ",
	fico_score__c                                             	string        comment "Def(En): Credit Score
Def(Th): คะแนน Credit",
...
Def(Th): วิธีการสื่อสาสรที่ต้องการ",
	finserv__conversiondatetime__c                            	string        comment "Def(En): Lead conversion Date and Time
...
Def(Th): การประเมินความน่าเชื่อถือทางเครดิต",
	finserv__creditscore__c                                   	string        comment "Def(En): Numerical rating of borrower’s creditworthiness.
...
Def(Th): ระบุที่อยู่อีเมล์ของบุคคลที่ได้รับการยืนยันแล้ว (true) หรือ ไม่ (false)",
	finserv__employedsince__pc                                	string        comment "Def(En): Year when client started with current employer.
...
Def(Th): ตัวเลข 4 ตัวท้ายของ SSN หรือ เลขประจำตัวผู้เสียภาษี",
	finserv__lastinteraction__c                               	string        comment "Def(En): Date of the last client meeting or call.
Def(Th): วันที่ของวันประชุมหรือโทรกับลูกค้าครั้งล่าสุด",
	finserv__lastreview__c                                    	string        comment "Def(En): Date of the last client review.
...
Def(Th): วันที่ทำรายการครั้งล่าสุด",
	finserv__lasttransactiondatejointowner__c                 	string        comment "Def(En): Date of the last transaction - joint owner.
Def(Th): วันที่ทำรายการครั้งล่าสุด - เจ้าของร่วม",
	finserv__lasttransactiondateprimaryowner__c               	string        comment "Def(En): Primary owner's last transaction date.
...
Def(Th): การดำรงตำแหน่งของบุคคลกับสถาบันการเงิน",
	finserv__lifetimevalue__c                                 	string        comment "Def(En): Individual’s lifetime net asset value to your financial institution.
...
Def(Th): ชื่อของ Mother's Maiden",
	finserv__networth__c                                      	string        comment "Def(En): Value of this individual’s assets minus liabilities.
Def(Th): มูลค่าทรัพย์สินของบุคคลนี้ลบด้วยหนี้สิน",
	finserv__nextinteraction__c                               	string        comment "Def(En): Date of the next meeting or call.
...
Def(Th): เหตุการณ์สำคัญในชีวิตครั้งต่อไป",
	finserv__nextreview__c                                    	string        comment "Def(En): Date of the next review.
...
Def(Th): หมายเหตุสำคัญที่เกี่ยวกับบุคคล",
	finserv__numberofchildren__pc                             	string        comment "Def(En): Number of children.
Def(Th): จำนวนบุตร",
	finserv__numberofdependents__pc                           	string        comment "Def(En): Number of dependents.
...
Def(Th): ผู้ใช้ภายในที่อ้างอิง",
	finserv__referrerscore__pc                                	string        comment "Def(En): A value of 0 to 100. A high score reflects a high rate of referral conversion.
Def(Th): ค่าตั้งแต่ 0 ถึง 100 คะแนนที่สูงสะท้อนถึงอัตราการแปลงผู้อ้างอิงที่สูง",
	finserv__relationshipstartdate__c                         	string        comment "Def(En): Date this individual started working with your company
...
Def(Th): กรอบเวลาการลงทุนของแต่ละบุคคล",
	finserv__totalaumjointowner__c                            	string        comment "Def(En): Total value of financial accounts that your company manages, for which this individual is the joint owner of jointly owned accounts.
Def(Th): มูลค่ารวมของบัญชีการเงินที่อยู่ใต้การจัดการของธนาคาร โดยบุคคลนี้เป็นเจ้าของร่วมของบัญชีที่เป็นเจ้าของร่วมกัน",
	finserv__totalaumprimaryowner__c                          	string        comment "Def(En): Total value of financial accounts that your company manages, for which this individual is the primary owner of either separately owned or jointly owned accounts.
...
Def(Th): มูลค่าบัญชีเงินฝากธนาคารทั้งหมด",
	finserv__totalbankdepositsjointowner__c                   	string        comment "Def(En): Value of all banking deposit accounts for which the client is the joint owner.
Def(Th): มูลค่าของบัญชีเงินฝากธนาคารทั้งหมดที่ลูกค้าเป็นเจ้าของร่วม",
	finserv__totalbankdepositsprimaryowner__c                 	string        comment "Def(En): Value of all banking deposit accounts for which the client is the primary owner.
Def(Th): มูลค่าของบัญชีเงินฝากธนาคารทั้งหมดที่ลูกค้าเป็นเจ้าของหลัก",
	finserv__totalfinacctsjointowner__c                       	string        comment "Def(En): Calculated total of jointly owned financial account balances for which the client is the joint owner.
Def(Th): ยอดรวมจากการคำนวณของยอดคงเหลือในบัญชีการเงินที่ลูกค้าเป็นเจ้าของร่วม",
	finserv__totalfinacctsprimaryowner__c                     	string        comment "Def(En): Calculated total of financial account balances where the client is the primary owner of either individually owned and jointly owned accounts.
...
Def(Th): ผลรวมของเงินฝากธนาคารทั้งหมดและเงินลงทุนทั้งหมด",
	finserv__totalheldfinacctsjointowner__c                   	string        comment "Def(En): Calculated total of held, jointly owned financial account balances for which the client is the joint owner.
Def(Th): ยอดรวมจากการคำนวณของยอดคงเหลือในบัญชีการเงินที่ถือครองและเป็นเจ้าของร่วมกันซึ่งลูกค้าเป็นเจ้าของร่วม",
	finserv__totalheldfinacctsprimaryowner__c                 	string        comment "Def(En): Calculated total of held financial account balances where the client is the primary owner of either individually owned or jointly owned accounts.
...
Def(Th): มูลค่ากรมธรรม์ประกันภัยทั้งหมด",
	finserv__totalinsurancejointowner__c                      	string        comment "Def(En): Value of all insurance policies for which the client is the joint owner.
Def(Th): มูลค่ากรมธรรม์ประกันภัยทั้งหมดที่ลูกค้าเป็นเจ้าของร่วม",
	finserv__totalinsuranceprimaryowner__c                    	string        comment "Def(En): Value of all insurance policies for which the client is the primary owner.
...
Def(Th): มูลค่าของบัญชีการลงทุนทั้งหมด",
	finserv__totalinvestmentsjointowner__c                    	string        comment "Def(En): Value of all investment accounts for which the client is the joint owner.
Def(Th): มูลค่าของบัญชีการลงทุนทั้งหมดที่ลูกค้าเป็นเจ้าของร่วม",
	finserv__totalinvestmentsprimaryowner__c                  	string        comment "Def(En): Value of all investment accounts for which the client is the primary owner.
...
Def(Th): มูลค่าของบัญชีหนี้สินทั้งหมด",
	finserv__totalliabilitiesjointowner__c                    	string        comment "Def(En): Total value of liabilities, for which this individual is the joint owner of jointly owned liabilities.
Def(Th): มูลค่ารวมของหนี้สินซึ่งบุคคลนี้เป็นเจ้าของร่วมของหนี้สินที่เป็นเจ้าของร่วมกัน",
	finserv__totalliabilitiesprimaryowner__c                  	string        comment "Def(En): Total value of liabilities, for which this individual is the primary owner of either separately owned or jointly owned liabilities.
...
Def(Th): มูลค่าของทรัพย์สินทางกายภาพทั้งหมด เช่น ของสะสมหรือโลหะมีค่า",
	finserv__totalnonfinancialassetsjointowner__c             	string        comment "Def(En): Value of all physical assets such as collectibles or precious metals for which the client is the joint owner.
Def(Th): มูลค่าของทรัพย์สินทางกายภาพทั้งหมด เช่น ของสะสมหรือโลหะมีค่าที่ลูกค้าเป็นเจ้าของร่วม",
	finserv__totalnonfinancialassetsprimaryowner__c           	string        comment "Def(En): Value of all physical assets such as collectibles or precious metals for which the client is the primary owner.
...
Def(Th): จำนวนบัญชีทางการเงินทั้งหมดของลูกค้าที่บริษัทของเราจัดการ",
	finserv__totalnumberoffinaccountsjointowner__c            	string        comment "Def(En): Total numbers of client's financial accounts as a joint owner.
Def(Th): จำนวนบัญชีทางการเงินของลูกค้าทั้งหมดในฐานะเจ้าของร่วม",
	finserv__totalnumberoffinaccountsprimaryowner__c          	string        comment "Def(En): Total numbers of client's financial accounts as a primary owner.
...
Def(Th): มูลค่าเครดิตคงค้างทั้งหมดของลูกค้าจากบัญชีการเงินที่บริษัทของเราจัดการ",
	finserv__totaloutstandingcreditjointowner__c              	string        comment "Def(En): Value of the total outstanding credit for which the client is a joint owner from the financial accounts that are managed by a specific firm.
Def(Th): มูลค่าของเครดิตคงค้างทั้งหมดที่ลูกค้าเป็นเจ้าของร่วมจากบัญชีทางการเงินที่ได้รับการจัดการโดยบริษัทเฉพาะ",
	finserv__totaloutstandingcreditprimaryowner__c            	string        comment "Def(En): Value of the total outstanding credit for which the client is the primary owner from the financial accounts that are managed by a specific firm.
Def(Th): มูลค่าของเครดิตคงค้างทั้งหมดที่ลูกค้าเป็นเจ้าของหลักจากบัญชีทางการเงินที่จัดการโดยบริษัทเฉพาะ",
	finserv__totalpremium__c                                  	string        comment "Def(En): Policyholder’s total annual premium.
Def(Th): เบี้ยประกันภัยรายปีรวมของผู้ถือกรมธรรม์",
	finserv__totalrevenue__c                                  	string        comment "Def(En): Total amount of revenue for a client.
...
Def(Th): เปอร์เซ็นต์ของสินทรัพย์ทางการเงินของบุคคลนี้ที่อยู่ใต้การบริหารจัดการของธนาคาร คำนวณเป็นเปอร์เซ็นต์ของบัญชีการเงินที่ถือครอง",
	finserv__weddinganniversary__pc                           	string        comment "Def(En): Date of wedding anniversary.
Def(Th): วันครบรอบแต่งงาน.",
	first_visited_date__c                                     	string        comment "Def(En): First Visited Date This Year
...
Def(Th): เป็นลูกค้าที่มีโอกาสในการขายหรือไม่",
	icr_ratio__c                                              	string        comment "Def(En): Interest Coverage Ratio
...
Def(Th): ประสบการณ์ในการ Import",
	income_to_bank_last_1_month__c                            	string        comment "Def(En): Income to Bank Last 1 Month
Def(Th): รายได้ต่อธนาคาร 1 เดือนที่ผ่านมา",
	income_to_bank_last_year__c                               	string        comment "Def(En): Income to Bank Last Year
...
Def(Th): ช่วงรายได้ต่อธนาคารปีที่แล้ว",
	income_to_bank_this_year__c                               	string        comment "Def(En): Income to Bank This Year
...
Def(Th): ช่วงรายได้ต่อธนาคารปีนี้",
	income_to_sme__c                                          	string        comment "Def(En): Income to SME
...
Def(Th): ลูกค้าได้มีการ convert แล้วหรือไม่",
	issuedandpaidup__c                                        	string        comment "Def(En): Shareholder Capital and Issued and paid-up share capital.
...
Def(Th): รหัสพนักงานที่ทำการตอบลีดคนล่าสุด",
	last_answer_lead_datetime__c                              	string        comment "Def(En): The lastest datetime that lead has been updated
Def(Th): วันที่ทำการตอบลีดล่าสุด",
	last_callreprt_opty_updated_date__c                       	string        comment "Def(En): The lastest datetime that call report has been updated
...
Def(Th): ชื่อพนักงานที่ทำการตั้งวงเงิน",
	last_setup_date__c                                        	string        comment "Def(En): The lastest datetime that application has been setup
...
Def(Th): ชื่อพนักงานที่ทำการเข้าเยี่ยมลูกค้าล่าสุด",
	last_visited_datetime__c                                  	string        comment "Def(En): The lastest datetime that customer has been visited
...
Def(Th): ชื่อ Lead for Sales บนลูกค้า",
	lead_sharing_expired_date__c                              	string        comment "Def(En): Lead Sharing Expired Date
...
Def(Th): มีการดำเนินการลีดของ CBS ไปแล้วหรือไม่",
	liabilitiesaccountpayable__c                              	string        comment "Def(En): Liabilities Accounts Payable
Def(Th): เจ้าหนี้การค้า",
	liabilitiescurrentliability__c                            	string        comment "Def(En): Liabilities Current Liability 
Def(Th): รวมหนี้สินหมุนเวียน",
	liabilitiesetc2__c                                        	string        comment "Def(En): Other Liabilities 2
Def(Th): รวมหนี้สินอื่นๆ 2",
	liabilitiesetc__c                                         	string        comment "Def(En): Other Liabilities
Def(Th): รวมหนี้สินอื่นๆ",
	liabilitieslongtermloan__c                                	string        comment "Def(En): Liabilities Long Term Loan
Def(Th): เงินกู้ยืมระยะยาว",
	liabilitiesnoncurrentliabilites__c                        	string        comment "Def(En): Non Current Liabilities 
Def(Th): รวมหนี้สินไม่หมุนเวียน",
	liabilitiesod__c                                          	string        comment "Def(En): Liabilities Overdraft
Def(Th): เงินเบิกเกินบัญชีและเงินกู้ยืม",
	liabilitiesshorttermloan__c                               	string        comment "Def(En): Liabilities Short Term Loan 
Def(Th): เงินกู้ยืมระยะสั้น",
	liabilitiestotalliability__c                              	string        comment "Def(En): Total Liabilities
Def(Th): รวมหนี้สิน",
...
Def(Th): ชื่อเล่นของลูกค้า",
	no_of_selection__c                                        	string        comment "Def(En): Count selection from RBS Port Selection
Def(Th): จำนวนพนักงานที่เลือกลูกค้าเข้า Port",
	number_of_product_holding__c                              	string        comment "Def(En): Number of Product Holding
...
Def(Th): Salesforce ID ของเจ้าของลูกค้า",
	pb_last_visited_datetime__c                               	string        comment "Def(En): PB Last Visited Date Time
...
Def(Th): สินเชื่อคงค้างสำหรับ report",
	retainedearning__c                                        	string        comment "Def(En): Retained Earning
...
Def(Th): รูปภาพตามความเสี่ยงที่รับได้",
	rm_contact__c                                             	string        comment "Def(En): Account Owner's Phone Number
...
Def(Th): ลูกค้ามีแนวโน้มขาย KCL ใช้แบบสอบถาม",
	sales_size__c                                             	string        comment "Def(En): Sales Size
Def(Th): ยอดขาย",
...
Def(Th): ลูกค้าเปราะบางมั้ย",
	sensitivecustomer__c                                      	string        comment "Def(En): Check Sensitive customer
Def(Th): ลูกค้าเปราะบางมั้ย",
...
Def(Th): สัญลักษณ์ของสถานะลูกค้า",
	stock_day_on_hand_days__c                                 	string        comment "Def(En): Stock Day on Hand Days
...
Def(Th): ชื่อของ TM ของเจ้าของลูกค้า",
	total_asset_exclude_land__c                               	string        comment "Def(En): Total assets excluding land
Def(Th): สินทรัพย์ทั้งหมดไม่รวมที่ดิน",
	total_benefit_contact__c                                  	string        comment "Def(En): Number of Total Benefir
Def(Th): จำนวนผลประโยชน์ทั้งหมด",
	total_contact_of_customers__c                             	string        comment "Def(En): Number of total contact
Def(Th): จำนวนผู้ติดต่อทั้งหมดบนลูกค้า",
	total_credit_limit__c                                     	string        comment "Def(En): Customer Credit Limit
...
Def(Th): ยอดเงินคงค้างทั้งหมดโดยดูจาก Commercial & Consumer",
	total_outstanding__c                                      	string        comment "Def(En): Sum of all Total Outstanding of CIS record
Def(Th): ผลรวมเงินคงค้างของลูกค้าโดยเช็คจาก CIS",
	total_sme_exposure__c                                     	string        comment "Def(En): Sum of out standing amount and limit amount for SME
...
Def(Th): ปริมาณของยอดเงินคงค้างกับวงเงินที่อนุมัติของ SME",
	total_sme_limit__c                                        	string        comment "Def(En): Approved credit amount for SME
...
Def(Th): ปริมาณวงเงินที่อนุมัติของ SME",
	total_sme_out__c                                          	string        comment "Def(En): Outstanding amount for SME
...
Def(Th): ปริมาณของยอดเงินคงค้างของ SME",
	totalliabilitiesandshareholders__c                        	string        comment "Def(En): Total liabilities and shareholder
Def(Th): รวมส่วนหนี้สินและผู้ถือหุ้น",
	totalshareholdersequity__c                                	string        comment "Def(En): Total shareholder equity
...
Def(Th): ประเภทของลูกค้า",
	used_reward_amount__c                                     	string        comment "Def(En): Amount of reward spent
...
Def(Th): ปริมาณของสินเชื่อคงค้าง",
	wallet_size_exposure__c                                   	string        comment "Def(En): Sum of Wallet Size Limit and Outstanding
Def(Th): ผลรวมของวงเงินที่อนุมัติกับยอดเงินคงค้าง",
	wallet_size_limit__c                                      	string        comment "Def(En): Approved credit limit
Def(Th): วงเงินที่อนุมัติ",
	wallet_size_outstanding__c                                	string        comment "Def(En): Outstanding amount
Def(Th): ยอดเงินคงค้าง",
	wallet_size_rbs__c                                        	string        comment "Def(En): Investment amount
Def(Th): ยอดเงินลงทุน",
...
Def(Th):Salesforce ID 18 หลักของ User ที่สร้าง record",
	account_extensions__r_createddate                         	string        comment "Def(En): Account Extension record created date
Def(Th): วันที่และเวลาที่สร้าง Record",
	account_extensions__r_account__c                          	string        comment "Def(En):Salesforce ID of Account
Def(Th):Salesforce ID 18 หลักของ Account",
	account_extensions__r_amount__c                           	string        comment "Def(En): Total amount
Def(Th): จำนวนเงินรวม",
	account_extensions__r_app__c                              	string        comment "Def(En): Investing Appliacation
Def(Th): App ที่ใช้ในการลงทุน",
	account_extensions__r_as_of_date_aum__c                   	string        comment "Def(En): Asset under manadement value as of date
Def(Th): มูลค่ารวมของ สินทรัพย์/หลักทรัพย์ในพอร์ตการลงทุน ณ วันที่",
	account_extensions__r_as_of_date_cheque__c                	string        comment "Def(En): Cheque date
...
Def(Th): รหัส Id ชองคนแก้ไขลูกค้าพึงระวังล่าสุด",
	account_extensions__r_c_watchlistupdatedate__c            	string        comment "Def(En): Date and time of each client's watchlist revision.
...
Def(Th): ประเภทการติดตามหนี้",
	account_extensions__r_committopaydate__c                  	string        comment "Def(En): The client's payment date.
...
Def(Th): มีบัญชีกองทุน",
	account_extensions__r_min_start_date_lead_from_website__c 	string        comment "Def(En): Min Start Date Lead From Website
...
Def(Th): ข้อความเพิ่มเติมสำหรับลูกค้า",
	account_extensions__r_pos_dt__c                           	string        comment "Def(En): Account Extension Pos Date
Def(Th): วัน Pos Date ของ Account Extension",
	account_extensions__r_priority__c                         	string        comment "Def(En):  Account Extension Priority
...
Def(Th): RM ต้องการทราบความเคลื่อนไหวด้านการเงิน (AUM) ของลูกค้าหรือไม่",
	account_extensions__r_rmf_purchase_amt__c                 	string        comment "Def(En): Total purchases RMF this year
...
Def(Th): รายละเอียดการขอสินเชื่อผลิตภัณฑ์ XpressLoan ควบคู่กับการดู All Product Code",
	account_extensions__r_ssf_purchase_amt__c                 	string        comment "Def(En): Total purchases SSF this year
Def(Th): ยอดซื้อ SSF ปีนี้",
	account_extensions__r_ssf_rmf_as_of_date__c               	string        comment "Def(En): SSF Purchase date
Def(Th): วันที่ซื้อ SSF",
	account_extensions__r_taxdeduction__c                     	string        comment "Def(En): Customer tax deduction
Def(Th): รูปแบบการลดหย่อนภาษีของลูกค้า",
	account_extensions__r_total_ssf_rmf_purchase_amt__c       	string        comment "Def(En): Total SSF & RMF this year
...
Def(Th):"
) using delta partitioned by (ptn_yyyy, ptn_mm, ptn_dd) location 'abfss://${storage_raw}/${catalog}/raw_rmst/rmst_customer' tblproperties (
